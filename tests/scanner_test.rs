...
async fn test_http_client_creation() -> Result<()> {
    ...
    let response = client.get(&format!("{}/test", mock_server.uri()))
        .send()
        .await?;
...
async fn test_check_signature() -> Result<()> {
    ...
    Mock::given(method("GET"))
        .and(path("/admin"))
        .respond_with(ResponseTemplate::new(200).set_body_string("<html><title>Admin Panel</title></html>"))
        .mount(&mock_server)
        .await;
    ...
    let matching_result = scanner::check_signature(&client, &admin_url, "<title>Admin Panel</title>").await?;
...
async fn test_scan_domain() -> Result<()> {
    ...
    Mock::given(method("GET"))
        .and(path("/login"))
        .respond_with(ResponseTemplate::new(200).set_body_string("<html><form><input name='username'><input name='password'></form></html>"))
        .mount(&mock_server)
        .await;
    ...
    let ruleset = RuleSet {
        rules: vec![
            Rule::new("Admin Panel", "/admin", "<title>Admin Panel</title>", "Admin panel detection", Severity::High),
            Rule::new("Login Form", "/login", "username.*password", "Login form detection", Severity::Medium),
            Rule::new("Not Found", "/not-found", "should-not-match", "Should not be found", Severity::Low),
        ],
    };
    ...
    let conn = db_conn.lock().await;
    let mut stmt = conn.prepare("SELECT domain, matched_path, rule_name, detected FROM findings")?;
